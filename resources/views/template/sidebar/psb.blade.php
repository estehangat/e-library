@if(in_array(Auth::user()->role->name,['ketuayys','pembinayys','direktur','kepsek','wakasek','am','aspv','ctl','ctm','fam','faspv','es','sek','keu']))
<div class="sidebar-heading">
    ...
    <li class="nav-item {{ (request()->is('kependidikan/psb*') && !request()->routeIs('kependidikan.psb.ortu*') && !request()->routeIs('kependidikan.psb.kunci*')) ? 'active' : '' }}">
        ...
        <div id="collapseSiswa" class="collapse {{ (request()->is('kependidikan/psb*') && !request()->routeIs('kependidikan.psb.ortu*') && !request()->routeIs('kependidikan.psb.kunci*')) ? 'show' : '' }}" aria-labelledby="headingBootstrap" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                @if(in_array(Auth::user()->role->name,['ketuayys','pembinayys','direktur','kepsek','wakasek','ctl','ctm','fam','faspv','es','sek','keu']))
                ...
                @endif
                @if(in_array(Auth::user()->role->name,['ketuayys','pembinayys','direktur','kepsek','wakasek','ctl','ctm','fam','faspv','es','keu']))
                ...
                @endif
                @if(in_array(Auth::user()->role->name,['ketuayys','pembinayys','direktur','kepsek','wakasek','ctl','ctm','es','sek','keu']))
                <hr class="sidebar-divider">
                    ...
                    @endif
                    @if(in_array(Auth::user()->role->name,['ketuayys','pembinayys','direktur','kepsek','wakasek','ctl','ctm','fam','faspv','keu']))
    ...
    @endif
    @if(in_array(Auth::user()->role->name,['sek','ketuayys','pembinayys','direktur','am','aspv']))
