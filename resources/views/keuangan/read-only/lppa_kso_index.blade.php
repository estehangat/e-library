                ...
                <div class="row mb-2">
                  ...
                  <div class="col-lg-6 col-md-6 col-12">
                    ...
                    <select aria-label="Tahun" name="tahun" class="form-control" id="yearOpt">
                      ...
                      @if($t->is_active == 1 || ($t->is_active != 1 && $t->whereHas('ppa',function($q)use($jenisAktif,$t){$q->where('academic_year_id',$t->id)->whereHas('jenisAnggaranAnggaran',function($q)use($jenisAktif){$q->where('budgeting_type_id',$jenisAktif->id);})->has('lppa');})->count()))
                      <option value="{{ $t->academicYearLink }}" {{ $tahun->id == $t->id ? 'selected' : '' }}>{{ $t->academic_year }}</option>
                      @endif
                      @endforeach
                    </select>
                    <a href="{{ route('lppa.index', ['jenis' => $jenisAktif->link]) }}" id="btn-select-year" class="btn btn-brand-purple ml-2 pt-2" data-href="{{ route('lppa.index', ['jenis' => $jenisAktif->link]) }}">Pilih</a>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div class="col-md-6 col-12 mb-3">
                        <div class="row py-2 rounded border border-light mr-2">
                            ...
                            <div class="col-4 d-flex justify-content-end align-items-center">
                                <a href="{{ route('lppa.index', ['jenis' => $jenisAktif->link, 'tahun' => $tahun->academicYearLink, 'anggaran' => $a->anggaran->link])}}" class="btn btn-sm btn-outline-brand-purple-dark">Pilih</a>
                            </div>
                        </div>
                    </div>
                    @endif
